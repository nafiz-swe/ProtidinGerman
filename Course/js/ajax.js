...
$(document).on("submit", "#examineeLoginFrm", function () {
  $.post("query/loginExe.php", $(this).serialize(), function (data) {
    ...
    else if (data.res == "denied") {
      Swal.fire(
        'Access Denied',
        'Your account status is "' + data.reason + '". Login is only allowed for Running or Completed students.',
        'warning'
      ).then(() => {
        // Optional: redirect to homepage or contact section
